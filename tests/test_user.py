...
@pytest.mark.parametrize(
    ...
    [
        ("admin_auth_2", 200),
        ("financial_auth_2", 403),
        ("user_auth_2", 403),
        ("user_owner_auth_2", 200),
        ("guest_auth_2", 401),
    ],
)
def test_patch_user(client, session_2, auth, status_code, request):
    authorization_header, user_id, _, _ = request.getfixturevalue(auth)
    existing_user = session_2.get(e.User, user_id)
    assert existing_user.first_name != "New First Name"
    assert existing_user.last_name != "New Last Name"
    new_user_data = {
...
def test_allowed_update_user_by_admin(client, session_2, admin_auth_2):
...
def test_allowed_update_user_by_user_owner(client, session_2, user_auth_2):
    existing_user = session_2.exec(
        select(e.User).where(e.User.email == "user3@example.com")
    ).one()
    assert existing_user.first_name != "John"
    new_user_data = {"first_name": "John"}
    header, _ = user_auth_2
    response = client.patch(
        f"/user/{existing_user.id}",
        json=new_user_data,
        headers=header,
...
def test_forbidden_update_by_user_owner(client, session_2, user_auth_2):
    existing_user = session_2.exec(
        select(e.User).where(e.User.email == "user3@example.com")
    ).one()
    assert existing_user.role != "admin"
...
def test_forbidden_update_by_user(client, session_2, user_auth_2):
    existing_user = session_2.exec(
...
def test_add_non_existing_initiative(client, session_2, admin_auth_2):
    ...
    authorization_header, email = admin_auth_2
    response = client.post("/user", json=user_data, headers=authorization_header)
